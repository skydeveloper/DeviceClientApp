...
# Функция за зареждане на конфигурацията от config.json
def load_config():
    try:
        with open('config.json', 'r', encoding='utf-8') as f:
            config = json.load(f)
            logger.info(f"Конфигурацията е заредена успешно: ID={config['device_id']}, Роля={config['device_role']}")
            return config
    except FileNotFoundError:
        logger.error("Критична грешка: Файлът 'config.json' не е намерен!")
        return None
    except json.JSONDecodeError:
        logger.error("Критична грешка: Файлът 'config.json' е невалиден JSON!")
        return None


config = load_config()
if not config:
    exit("Приложението не може да стартира без валиден config.json")


# Функция за динамично зареждане на handler според ролята
def load_handler(role_name, role_config):
    try:
        module_name = f"roles.{role_name}_handler"
        class_name = f"{role_name.capitalize()}Handler"
        handler_module = importlib.import_module(module_name)
        handler_class = getattr(handler_module, class_name)
        instance = handler_class(role_config)
        logger.info(f"Handler '{class_name}' е зареден успешно.")
        return instance
    except (ImportError, AttributeError) as e:
        logger.error(f"Не може да се зареди handler за роля '{role_name}': {e}")
        return None


# Зареждаме конкретния handler при стартиране
task_handler = load_handler(config.get('device_role'), config.get('role_specific_config', {}))

app = Flask(__name__)
app.secret_key = "device_client_super_secret_key"

# Настройки от конфигурацията
ASMG_REPORT_URL = "http://localhost:5000/api/device/report"
DEVICE_ID = config.get('device_id', 'Unknown_Device')

# Глобално състояние
current_task_info = {
    "module_serial_numbers": ["", "", "", ""], "active_slots": [False, False, False, False],
    "item_name": "", "status_message": "Готов за нова задача", "error_message": "",
    "is_busy": False, "last_asmg_command": None
}
...
def report_to_asmg(report_type, message, payload=None):
    ...
    try:
        ...
        logger.info(f"Отговор от ASMg: {response.status_code}")
...
def execute_task_and_report(serials, slots, item):
    ...
    result_data = programmer_interface.start_actual_task(task_handler, serials, slots, item)
...
def device_client_index():
    ...
    return render_template('device_client_index.html',
                           task_info=current_task_info,
                           device_config=config,
                           task_history=list(task_history))


# !!! ТУК БЕШЕ ЛИПСВАЩИЯТ КОД !!!
@app.route('/api/start_task', methods=['POST'])
def api_start_task_from_asmg():
    global current_task_info
    data = request.get_json()

    # Добавяме лог веднага щом получим заявката
    logger.info(f"DeviceClient: Получена API команда: {data}")
...
def get_results():
    """REST API ендпойнт за извличане на историята с резултати."""
...
if __name__ == '__main__':
    if not task_handler:
        logger.error(
            "Приложението спира, защото не може да зареди нужния handler. Проверете 'device_role' в config.json.")
    else:
